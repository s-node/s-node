...
class XT_Lang {
	...
	function XT_Lang($language){
		...
		if((is_file(LANG_DIR . $language . '.lang.php') && array_key_exists($language, $GLOBALS['cfg']->getLangs())) || $language == '-1'){
			$this->_lang = $language;
		}
		if($this->_lang != '-1'){
	...
	function msg($msg,$useLiveTranslate = false){
		if((isset($this->_privateMessages[$msg]) || isset($this->_systemMessages[$msg])) && $this->_lang != '-1'){
