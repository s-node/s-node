...
// Strasse pruefen
if(empty($register['street'])) {
    $errors['street'] = "No street given";
}

// Nummer pruefen
if(empty($register['street_nr'])) {
    $errors['street_nr'] = "No nummer given";
}

// PLZ pruefen
if(empty($register['postalCode'])) {
    $errors['postalCode'] = "No zip given";
}

// Ort pruefen
if(empty($register['city'])) {
    $errors['city'] = "No city given";
}

// E-Mail Adresse pruefen
if(empty($register['email'])) {
    $errors['email'] = "No email given";
}
elseif (!XT::checkEmail(trim($register['email']))) {
    $errors['email'] = "No valid email given";
}

// Telefon pruefen
if(empty($register['tel'])) {
    $errors['tel'] = "No tel given";
}
