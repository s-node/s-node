...
<html xmlns="http://www.w3.org/1999/xhtml" xml:lang="{$SYSTEM_LANG}" lang="{$SYSTEM_LANG}">
 ...
 <body>
  {plugin package="ch.iframe.snode.shop" module="enabler"}
  {plugin package="ch.iframe.snode.toolbox" module="viewer"}
    <a name="top"></a>
    <div id="container">
     <div id="header"><a href="{$smarty.server.PHP_SELF}"><img src="/images/default/head.gif" alt="S-NODE XT default" /></a>{image id=$HEADIMAGE version="orig" class="headimage"}</div>
     <div id="topnav">{plugin package="ch.iframe.snode.navigation" module="tree" depth=1 open_depth=1 node="10000"}</div>
     <div id="sidenav"><!--%%XT_ZONE_LEFT%%-->
     {plugin package="ch.iframe.snode.navigation" module="tree" depth=2 start_level=2 node="10000" style="sidenav.tpl"}</div>
     <div id="content">